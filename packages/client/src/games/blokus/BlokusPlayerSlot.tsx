import { computeRemainingCells } from "@bodobako/shared";
import type { PlayerSlotProps } from "../../components/GameSidebar/PlayerCard";
import { useRoom } from "../../context/RoomContext";
import { BLOKUS_COLORS } from "./constants";

export function BlokusPlayerSlot({ playerId: slotPlayerId }: PlayerSlotProps) {
  const { gameState } = useRoom();
  const state = gameState?.gameId === "blokus" ? gameState.state : null;
  if (!state) return null;

  const playerIndex = state.playerIds.indexOf(slotPlayerId);
  if (playerIndex === -1) return null;

  // このプレイヤーが所有するカラーインデックス（0-3）
  ...
  return (
    <div className="flex gap-1 flex-wrap items-center mt-0.5">
      {displayColors.map((colorIndex) => {
        const remaining = computeRemainingCells(state, colorIndex);
        const isEliminated = state.eliminated[colorIndex];
        const accentColor = BLOKUS_COLORS[colorIndex].fill;
        return (
          <span
            key={colorIndex}
          ...
          >
            ...
            />
            <span className="text-[0.68rem] text-slate-500">{remaining}マス</span>
          </span>
        );
      })}
    </div>
